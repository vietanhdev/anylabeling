...
class Canvas3D(QtInteractor):
    ...
    def __init__(self, parent=None):
        ...
        # Setup plotter. enable_trackball_style requires an interactive
        # render-window iren, which isn't present in headless test envs
        # (vtk-osmesa, no display) — guard so the widget can still be
        # constructed for unit tests of the data-model layer.
        try:
    ...
    def _iren_interactor(self):
        """Return the live VTK interactor, or None in headless contexts."""
        try:
            iren = self.iren.interactor if self.iren is not None else None
        except RuntimeError:
            iren = None
        return iren
