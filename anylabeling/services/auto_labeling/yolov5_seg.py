...
class YOLOv5Segmentation(Model):
    ...
    def post_process(self, input_image, outputs):
        ...
        # box preprocessing
        outputs[0][0, :, 0] = (
            outputs[0][0, :, 0] - 0.5 * outputs[0][0, :, 2]
        ) * x_factor
        outputs[0][0, :, 1] = (
            outputs[0][0, :, 1] - 0.5 * outputs[0][0, :, 3]
        ) * y_factor
        ...
        for i in selected:  # loop through selected
            ...
            _, score, _, label = cv2.minMaxLoc(
                scores[i]
            )  # get score and classId
            ...
            if (
                ...
                if self.output_mode == "polygon":
                    ...
                    mask = (
                        mask >= self.config["mask_threshold"]
                    )  # filtering mask by tresh
                    ...
                    gray = cv2.cvtColor(
                        black.astype(np.uint8), cv2.COLOR_BGR2GRAY
                    )
                    ...
                    shape.add_point(
                        QtCore.QPointF(box[0] + box[2], box[1] + box[3])
                    )
